...
function renderEngines() { els.grid.replaceChildren(...engines.map((e,i)=>{const label=document.createElement('label'); label.className='engine'; label.innerHTML=`<input type="checkbox" data-engine="${e.id}" ${i<4?'checked':''}><div class="engine-body"><div class="engine-title"><strong>${e.name}</strong><span class="badge">${e.badge}</span></div><span class="engine-desc">${e.description}</span><span class="mode">${e.mode==='url'?'⚡ 圖片 URL 可直接啟動':'↗ 開啟網站後手動上傳'}</span></div>`; return label;})); updateHint(); }
...
async function deleteRemote(){ if(!state.remoteUrl)return; const remote=state.remoteUrl; state.remoteUrl=null; state.remoteExpiresAt=0; if(state.cleanupTimer)clearTimeout(state.cleanupTimer); try{await fetch(remote,{method:'DELETE',keepalive:true});}catch{} }
function scheduleRemoteCleanup(){ if(state.cleanupTimer)clearTimeout(state.cleanupTimer); if(!state.remoteUrl)return; const delay=Math.max(5000,state.remoteExpiresAt-Date.now()+1000); state.cleanupTimer=setTimeout(()=>{deleteRemote();},delay); }
...
async function ensureRemoteUrl(){if(state.remoteUrl&&Date.now()<state.remoteExpiresAt-30000)return state.remoteUrl;if(!state.image?.blob)return els.url.value.trim();setStatus('正在建立短期暫存圖片 URL…');const r=await fetch('/api/image',{method:'POST',headers:{'content-type':state.image.blob.type||'image/webp'},body:state.image.blob});if(!r.ok)throw new Error('暫存圖片服務尚未設定，請先綁定 Cloudflare R2。');const data=await r.json();state.remoteUrl=data.url;state.remoteExpiresAt=data.expiresAt||Date.now()+9*60*1000;scheduleRemoteCleanup();setStatus('暫存 URL 已建立。',true);return state.remoteUrl;}
function openPopup(url){return !!window.open(url,'_blank','noopener,noreferrer');}
async function searchOne(engine){let url=engine.buildManualUrl();let method='manual';if(engine.mode==='url'){const source=await ensureRemoteUrl();if(!source)throw new Error('沒有可用的圖片 URL。');url=engine.buildUrl(source);method='url';}const opened=openPopup(url);return{engine,opened,method,url};}
async function runSearch(ids){if(!ids.length){setStatus('請至少選擇一個搜尋引擎。');return;}if(!state.image?.blob&&!els.url.value.trim()){setStatus('請先加入圖片。');return;}els.results.replaceChildren();const selected=selectedEngines(ids);for(const engine of selected){const row=document.createElement('div');row.className='result';const main=document.createElement('div');main.className='result-main';const title=document.createElement('strong');title.textContent=engine.name;const sub=document.createElement('span');sub.textContent='準備中…';main.append(title,sub);const button=document.createElement('button');button.className='secondary';button.type='button';button.textContent='開啟';row.append(main,button);els.results.append(row);const open=async()=>{try{const r=await searchOne(engine);sub.textContent=r.opened?(r.method==='url'?'已以圖片 URL 開啟':'已開啟搜尋頁，請上傳圖片'):'瀏覽器阻擋新視窗';}catch(err){sub.textContent=err.message||'開啟失敗';}};button.addEventListener('click',open);await open();}}
...
els.selectAll.addEventListener('click',()=>{els.grid.querySelectorAll('input').forEach(x=>x.checked=true);updateHint();});els.selectNone.addEventListener('click',()=>{els.grid.querySelectorAll('input').forEach(x=>x.checked=false);updateHint();});els.grid.addEventListener('change',updateHint);
els.selected.addEventListener('click',()=>runSearch(checkedIds()));els.all.addEventListener('click',()=>{els.grid.querySelectorAll('input').forEach(x=>x.checked=true);updateHint();runSearch(checkedIds());});
