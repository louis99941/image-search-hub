...
async function deleteRemote() {
  ...
  const remote = state.remoteUrl; state.remoteUrl = null; state.remoteExpiresAt = 0;
...
async function loadFromUrl() {
  ...
  try { setStatus('正在下載圖片到瀏覽器…'); const r = await fetch(url, { mode: 'cors' }); if (!r.ok) throw new Error(`HTTP ${r.status}`); const b = await r.blob(); await setImage(b, url.split('/').pop()?.split('?')[0] || 'URL image'); }
...
async function ensureRemoteUrl() {
  ...
  if (!r.ok) throw new Error('暫存圖片服務尚未設定，請先綁定 Cloudflare R2。');
  const data = await r.json(); state.remoteUrl = data.url; state.remoteExpiresAt = data.expiresAt || Date.now() + 9 * 60 * 1000; scheduleRemoteCleanup();
  setStatus('暫存 URL 已建立。', true); return state.remoteUrl;
}
...
window.addEventListener('beforeunload', () => { if (state.remoteUrl) navigator.sendBeacon?.(state.remoteUrl, new Blob([], { type: 'application/octet-stream' })); });
